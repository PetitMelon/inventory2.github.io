...
function toDailyPace(amount, period) {
  ...
  switch (period) {
    case 'week':  return a / 7;
    case 'month': return a / 30;
...
function paceLabel(amount, period, unit) {
  const u = unit || '個';
  switch (period) {
    case 'week':  return `${amount}${u}／週`;
    case 'month': return `${amount}${u}／月`;
    default:      return `${amount}${u}／日`;
  }
}
...
async function runAutoConsumption() {
  ...
  for (const item of items) {
    ...
    if (item.lastAutoDate === todayStr) continue;

    // 経過日数を計算
    const elapsedDays = item.lastAutoDate
      ? Math.max(1, daysBetween(item.lastAutoDate, todayStr))
      : 1;
    ...
    if (item.autoMode === 'learn') {
      ...
      dailyPace = calcLearnedPace(hist)
        ?? toDailyPace(item.manualPace ?? 1, item.pacePeriod ?? 'day');
    } else {
      dailyPace = toDailyPace(item.manualPace ?? 1, item.pacePeriod ?? 'day');
    }

    const totalConsume = dailyPace * elapsedDays;

    if (item.mode === 'percent') {
      const newPct = Math.max(0, Math.round((item.percent ?? 100) - totalConsume));
      await update(ref(db, `inventory/items/${item._key}`), { percent: newPct, lastAutoDate: todayStr });
    } else {
      const newStock = Math.max(0, Math.round(((item.stock ?? 0) - totalConsume) * 10) / 10);
      await update(ref(db, `inventory/items/${item._key}`), { stock: newStock, lastAutoDate: todayStr });
    }
  }
}
...
async function initDefaultItems() {
  const defaults = [
    { name:'トイレットペーパー', cat:'日用品',    mode:'count',   stock:1,  threshold:3,  unit:'個', emoji:'🧻', img:null, autoEnabled:false, price:null, url:null },
    { name:'シャンプー',         cat:'日用品',    mode:'count',   stock:5,  threshold:2,  unit:'本', emoji:'🧴', img:null, autoEnabled:false, price:null, url:null },
    { name:'ワンちゃんのエサ',   cat:'ペット用品', mode:'count',   stock:2,  threshold:2,  unit:'袋', emoji:'🐾', img:null, autoEnabled:false, price:null, url:null },
    { name:'猫砂',               cat:'ペット用品', mode:'percent', percent:75, pctThreshold:50, emoji:'🐱', img:null, autoEnabled:false, price:null, url:null },
    { name:'食器用洗剤',         cat:'洗剤・清掃', mode:'count',   stock:1,  threshold:2,  unit:'本', emoji:'🧹', img:null, autoEnabled:false, price:null, url:null },
    { name:'お米',               cat:'食品・飲料', mode:'percent', percent:50, pctThreshold:25, emoji:'🍚', img:null, autoEnabled:false, price:null, url:null },
  ];
...
function cardHTML(item) {
  ...
  const priceHTML = item.price != null && item.price !== ''
  ...
  if (item.mode === 'percent') {
    ...
    const pctSteps = [100,75,50,25,0].map(p =>
      `<button class="pct-step-btn${pct===p?' current':''}" data-pct-step="${item._key}" data-pct="${p}">${p}%</button>`
    ).join('');
    controlHTML = `
      <div class="percent-bar-wrap">
        <div class="percent-bar-bg"><div class="percent-bar-fill ${fillCls}" style="width:${pct}%"></div></div>
        <div class="percent-num-row">
          <span class="percent-num num-${st}">${pct}</span>
          <span class="percent-num-unit">% 残量</span>
        </div>
  ...
  // 購入ボタン
  const buyBtn = item.url
    ? `<a class="buy-btn" href="${item.url}" target="_blank" rel="noopener noreferrer" data-buy="true">🛒 購入サイトへ</a>`
    : '';

  // 自動消費バッジ
  const autoBadge = item.autoEnabled
    ? `<div class="auto-badge ${item.autoMode==='learn'?'learn':'on'}">${item.autoMode==='learn'?'🧠 自動学習':'⏱ '+paceLabel(item.manualPace??1, item.pacePeriod??'day', item.unit)}</div>`
    : '';
...
document.getElementById('mode-selector').addEventListener('click', e => {
  const btn = e.target.closest('.mode-btn'); if (btn) setMode(btn.dataset.mode);
});
document.getElementById('percent-selector').addEventListener('click', e => {
  const btn = e.target.closest('.pct-btn'); if (btn) setPct(parseInt(btn.dataset.pct));
});
document.getElementById('pct-threshold-selector').addEventListener('click', e => {
  const btn = e.target.closest('.pct-th-btn'); if (btn) setPctThreshold(parseInt(btn.dataset.th));
});
...
function openAdd() {
  ...
  document.getElementById('f-pace-period').value       = 'day';
...
function openEdit(key) {
  ...
  document.getElementById('f-pace-period').value       = item.pacePeriod ?? 'day';
